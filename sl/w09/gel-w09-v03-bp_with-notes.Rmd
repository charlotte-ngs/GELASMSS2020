...
\begin{itemize}
\item Today, we are looking into model selection and into statistical modelling.
\item Last week we saw that in our breeding program, we need statistical modelling to predict breeding values. 
\item The question is why do we need statistical modelling? Would it not be possible to find the best animals without statistics?
\end{itemize}
}
...
\begin{itemize}
\item Some people believe that statistics is not useful, or that statistics is only used to fix problems in experimental design
\item I had a lecturer in physiology, and he told us that for him it is enough to look at a diagram to see differences in control versus treatment
\end{itemize}
